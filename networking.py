...
TEMP_FEEDS = ["temperature-zone-1","temperature-zone-2","temperature-zone-3"]
SETPOINT_FEEDS = ["set-point-zone-1","set-point-zone-2","set-point-zone-3"]
COOLING_SECONDARY_FEED = ["cooling-secondary"]
HEATING_SECONDARY_FEED = ["heating-secondary"]
COOLING_PRIMARY_FEED = ["cooling-primary"]
HEATING_PRIMARY_FEED = ["heating-primary"]
DAMPER_FEEDS = ["damper-zone-1","damper-zone-2","damper-zone-3"]
SET_DAMPER_FEEDS = ["set-damper-zone-1","set-damper-zone-2","set-damper-zone-3"]
AUTOMATIC_FEED = ["automatic-mode"]
FAN_FEED = ["Fan"]
...
def socket_connect():
    ...
    while not connected:
        ...
        print("Connecting to " + str(SECONDARY_HOST))
...
def socket_listen(callback_function):
    ...
    try:
        my_socket.bind((SECONDARY_HOST,TCP_PORT))
        my_socket.listen(1)
    except OSError as e:
...
def socket_send_message(msg):
    ...
    if len(msg) <= SOCKET_MESSAGE_MAX_LENGTH:
        ...
        while totalsent < len(msg):
            sent = my_socket.send(msg[totalsent:])
            if sent == 0:
...
def mqtt_initialize():
    ...
    SET_DAMPER_FEEDS.extend([f"set-damper-zone-{i}" for i in range(1, num_zones + 1)])

    # Print the defined feeds for debugging purposes
    print(f"Feeds available: {FAN_FEED}, {TEMP_FEEDS}, {SETPOINT_FEEDS}, {DAMPER_FEEDS}, {COOLING_SECONDARY_FEED}, {COOLING_PRIMARY_FEED}, {HEATING_SECONDARY_FEED}, {HEATING_PRIMARY_FEED}, {AUTOMATIC_FEED}, {SET_DAMPER_FEEDS}")
...
def loop():
    ...
    global _prev_time, my_socket, socket_buffer, new_socket, _socket_callback, _socket_listening
    ...
    if node_type != NODE_TYPE_SIMULATED:
        if _socket_listening is True:
            global new_socket, my_socket, socket_buffer
            try:
                (new_s,(add, port)) = my_socket.accept()
            ...
            if new_socket:
                try:
                    n = new_socket.recv_into(socket_buffer)
